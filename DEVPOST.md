...
- **Uncertainty you can see.** A single projected line quietly claims the forecast is exact, and forecasts never are. Switch this on and the projection runs 32 more times per fire, each one nudging the wind within the range the forecast could plausibly be wrong by. The area all those attempts cover gets shaded in cyan. A narrow shape means the wind agrees with itself and the projection is worth trusting. A wide one means it could genuinely go several ways.
